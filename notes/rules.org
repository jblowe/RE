...
** Next steps (in order)
* TODO Give a go at implementing 3d ToC with chronology numbers, see if it jives with the algorithm.
* TODO Write code to go from a 2d ToC with no chronology numbers to a rule view.
* TODO Write code to go from a partially ordered rule view to a 3d TOC.
* TODO Maybe incorporate featural analysis to make rule view without chronology more useful.
