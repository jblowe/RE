...
<xsl:template match="/">
	<html>
		<head>
			...
			.entry2              { margin-left:  15px;}
			.entry2:first-letter { margin-left: -30px;}
...
<!-- sous-entree -->
<xsl:template match="ENTRY2">
	<xsl:apply-templates select="@*"/>
	<div class="entry2"><xsl:apply-templates/></div>
</xsl:template>
...
<xsl:template match="dff|dfe|nag|mmnag|rajnag|dfn">
	<xsl:apply-templates select="@*"/>
	<xsl:choose>
		...
		<xsl:when test="local-name(.) = 'mmnag'"><span class="small"><xsl:text> NEP </xsl:text></span></xsl:when>
		<xsl:when test="local-name(.) = 'rajnag'"><span class="small"><xsl:text> NEP </xsl:text></span></xsl:when>
