...
parameters_file = os.path.join(project_dir, f'{args.project}.default.parameters.xml')
...
if args.coverage:
    ...
    coverage_xml_file = os.path.join(project_dir, f'{args.project}.mel.statistics.xml')
    ...
    if need_to_compare:
        ...
        analysis_file = os.path.join(project_dir, f'{args.project}.{args.run}.analysis.txt')
        ...
        sets_file = os.path.join(project_dir, f'{args.project}.{args.run}.sets.txt')
        RE.dump_sets(B, sets_file)
        print(f'wrote {len(B.forms)} text sets to {sets_file}')
        sets_xml_file = os.path.join(project_dir, f'{args.project}.{args.run}.sets.xml')
        RE.dump_xml_sets(B, sets_xml_file)
        print(f'wrote {len(B.forms)} xml sets to {sets_xml_file}')
        # failures (no parses)
        failures_xml_file = os.path.join(project_dir, f'{args.project}.{args.run}.failures.sets.xml')
        ...
        # isolates
        isolates_xml_file = os.path.join(project_dir, f'{args.project}.{args.run}.isolates.sets.xml')
        ...
        stats_xml_file = os.path.join(project_dir, f'{args.project}.{args.run}.upstream.statistics.xml')
