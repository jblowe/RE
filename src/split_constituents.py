...
# segment inventory and syllable canon
c = 'ŋŋɦɓbcdɗghjklmnpqrstwzʰʔʣʦ'
v = 'aeiouuʊɛːɨɔAôˀ'
f = 'st'
SYLLABLE_CANON = rf'^([{c}]*)([{v}]+[{c}]*?)([{f}])?\b'
# other dataset specific parameters
NUMBER_OF_CONSTITUENTS = 3
CONSTITUENT_TYPES = 'O R S'.split(' ')
PROTO_LANGUAGE = 'PK'
...
def parsew(s):
    if s:
        ...
        if m:
            ...
            if len(result) > NUMBER_OF_CONSTITUENTS and result[NUMBER_OF_CONSTITUENTS] is None:
                result[NUMBER_OF_CONSTITUENTS] = '0'
            return result
        else:
            print('no parse', s)
            return [''] * NUMBER_OF_CONSTITUENTS
    else:
        return [''] * NUMBER_OF_CONSTITUENTS
        ...
        for i, row in enumerate(r):
            ...
            for word in words:
                ...
                try:
                    ...
                    pass
                out = out + parsed_form
                [correspondence[CONSTITUENT_TYPES[c]].append(parsed_form[c]) for c in range(NUMBER_OF_CONSTITUENTS)]
