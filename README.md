# RE
The Reconstruction Engine, a computer implementation of the comparative method

Two versions exist here:

* RE7: an incomplete re-implementation in PERL of the original Spitbol version.
* RE8.2: a Python version implemented from scratch based on the design and data of the RE7 version

See individual directories for instructions on installation and operation.
